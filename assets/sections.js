...
window.listCollection = {
  init() {
    $('.js-collection-slider .collection-slider').each((index, value) => {
      const productsPerSlide = $(value).data('collection-per-slide');
      const productsLimit = $(value).data('collection-limit');
      const productsAvailable = $(value).data('collection-available');
      let cellAlign = '';
      let draggable = false;
      let prevNext = false;
      let wrapAround = false;
      ...
      $(value).addClass('collection-initialized');
    });
  },
  unload($target) {
    const $slider = $target.find('.js-collection-slider');
  ...
  init() {
    ...
    if ($(window).width() > 798) {
      setInterval(function(){
        if($('.header-main.nav-fixed--true').length) {
          $('.sub-sticky').css('margin-top',$('.header-main.nav-fixed--true').height());
        }
      }, 500);
    }
...
//custom js
//show and hide desc in collection page
(function($){
  $(function(){
    $('.collection .btn-overflow').on('click',function (e){
      e.preventDefault();
      $('.collection .overlay').toggleClass('show_overlay');
      $('.collection_description').toggleClass('show_desc');
      if($('.collection_description').hasClass('show_desc')){
        $(this).text('see less');
      }else{
        $(this).text('see more');
      }
